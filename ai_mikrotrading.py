...
DECISION_LOG_FILE = "quant_v17_micro_log.csv"
MAX_LOT_SMALL_ACCOUNT = 0.10 # Hard cap lot untuk akun kecil
...
# ==========================================
db_conn = sqlite3.connect("quant_production_log_micro.db", check_same_thread=False)
...
def account_kill_switch():
    ...
    if hist:
        ...
        # PERBAIKAN 3: Kill Switch diturunkan menjadi 3% dari 5% untuk akun micro
        if pnl < -0.03 * acc.equity:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] !!! GLOBAL KILL SWITCH !!! Drawdown > 3%")
            return True
    return False
...
def train_model(sym, config):
    df = get_safe_rates(sym, mt5.TIMEFRAME_M15, 4000) 
    ...
    new_threshold = np.quantile(np.max(val_probs, axis=1), 0.50)
    ...
    print(f"[{sym}] V17 Micro Trained | Thresh: {threshold:.4f}")
...
def execute_trade(sym, action, raw_pred, conf, m_data, adaptive_threshold, vol_ratio, spread_points, atr_now):
    ...
    # PERBAIKAN 1: Eksekusi Gatekeeper USD Exposure
    current_usd_exposure = get_usd_net_exposure()
    new_usd_impact = get_usd_impact(sym, action)
    if abs(current_usd_exposure + new_usd_impact) > MAX_USD_EXPOSURE:
        log_decision(sym, action, raw_pred, conf, adaptive_threshold, vol_ratio, spread_points, "REJECT_USD_EXPOSURE")
        return
    ...
    risk_percent = np.clip(risk_percent, BASE_RISK_PERCENT, BASE_RISK_PERCENT * 1.5) 
    ...
    lot = min(lot, MAX_LOT_SMALL_ACCOUNT)
    ...
    if res and res.retcode == mt5.TRADE_RETCODE_DONE and verify_pos:
        ...
        print(f"[{sym}] {action} VERIFIED | Lot: {lot} | Expected Edge: {expected_edge:.2f} | Latency: {latency:.3f}s")
    else:
...
# 5. MAIN EVENT DAEMON
# ==========================================
print("Initializing Micro Quant Models...")
...
print("\n--- QUANT ENGINE V17.0 MICRO DEPLOYED ---")
...
while True:
    ...
    # PERBAIKAN 2: Matrix Correlation rutin di-update setiap 1 jam (lebih responsif)
    if datetime.now() - last_corr_update > timedelta(hours=1):
        update_correlation_matrix()
    ...
    if df_ref is not None and df_ref['time'].iloc[0] != last_candle_time:
        ...
        if 2 <= datetime.now(timezone.utc).hour <= 21:
            for sym, m_data in list(models_vault.items()):
                ...
                atr_median_500 = df_inf['ATR_14'].rolling(500).median().iloc[-1]
                if vol_now < atr_median_500 * 0.5:
                    log_decision(sym, "NONE", 0, 0.0, m_data['threshold'], 0.0, 0.0, "SKIP_LOW_ATR")
                    continue
                
                vol_mean = df_inf['ATR_14'].rolling(200).mean().iloc[-1]
                vol_ratio = vol_now / vol_mean
                
                # PERBAIKAN 2: Batas shock volatilitas diturunkan ke 1.5 agar trigger Matrix Update lebih cepat
                if vol_ratio > 1.5 and datetime.now() - last_corr_update > timedelta(minutes=10):
                    update_correlation_matrix()
                    last_corr_update = datetime.now()
                ...
                if conf >= adaptive_threshold and raw_pred != 0:
                    ...
                    if raw_pred == 0 and conf >= adaptive_threshold:
                        log_decision(sym, "NONE", raw_pred, conf, adaptive_threshold, vol_ratio, spread_points, "SKIP_SIDEWAYS")
                    else:
